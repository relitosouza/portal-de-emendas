...
function getQueryString(searchParams: Record<string, string | string[] | undefined>) {
...
export default async function RelatorioIndicacoesPage(props: Props) {
    ...
    const visibleAmendments = amendments.filter((item) => matchesFilters(item, filters));
    const totalValue = visibleAmendments.reduce((acc, item) => acc + parseCurrency(item.valorAutorizado || item.valor), 0);
    const today = new Date().toLocaleDateString("pt-BR");
    const backQuery = getQueryString(searchParams);
        ...
        <>
            <div className="no-print w-full bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center sticky top-0 z-50 shadow-sm">
                <div className="flex items-center gap-3">
                    <Link
                        href={`/projetos${backQuery ? `?${backQuery}` : ""}`}
                ...
                <div className="print-container max-w-[980px] mx-auto my-8 print:my-0 bg-white shadow-xl print:shadow-none p-10 border border-slate-200 print:border-none min-h-[1123px] relative flex flex-col">
                    ...
                    <header className="flex items-start justify-between gap-6 border-b-2 border-blue-600 pb-5 mb-6 relative z-10">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-[10px] text-slate-400">Consulta filtrada</p>
                        </div>
                    </header>
                    ...
                    <section className="mb-8">
                        <div className="flex items-center justify-between gap-4 border border-slate-200 rounded-lg p-4 bg-white">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-sm font-bold font-mono text-slate-800">RELATORIO-DE-INDICACOES</p>
                                <p className="text-[10px] text-slate-400 mt-1">Portal das Emendas · Prefeitura de Osasco</p>
                            </div>
                        </div>
                    </section>
                    ...
                    <footer className="mt-auto pt-8 border-t border-slate-200 flex justify-between items-end">
                        <div className="max-w-[520px]">
                            <p className="text-[10px] text-slate-500 mb-4 leading-relaxed">
                                Este documento consolida as indicações exibidas a partir dos filtros aplicados na consulta.
                                A verificação das informações pode ser feita diretamente no Portal das Emendas da Prefeitura de Osasco.
                            </p>
                        </div>
